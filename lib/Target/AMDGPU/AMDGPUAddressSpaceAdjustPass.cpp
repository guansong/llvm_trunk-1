...
namespace {
  ...
  void updateGEPWithNewOperand(GetElementPtrInst * GEP, Value * oldOperand, Value * newOperand, InstUpdateWorkList * updatesNeeded)
  {
    ...
    DEBUG(
        llvm::errs() << "future type: "; futureType->dump(); llvm::errs() << "\n";
        llvm::errs() << "address space: " << GEP->getAddressSpace() << "\n";
        llvm::errs() << "indexed type: "; 
        if (GEP->getIndexedType(oldOperand->getType(), Indices)) GEP->getIndexedType(oldOperand->getType(), Indices)->dump(); 
        llvm::errs() << "\n";
        );
...
const char * PromoteGlobals::getPassName() const {
  return "AMDGPU Promote Pointer to Global Pass";
}
...
bool PromoteGlobals::runOnModule(Module& M)
{
  ...
  for (kernel_iterator F = foundKernels.begin(), Fe = foundKernels.end();
    ...
    DEBUG(llvm::errs() << "escapename: " << escapeName(promoted->getName().str()) << "\n";);
